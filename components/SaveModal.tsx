...
export default function SaveModal({ photoId, onClose, onSaved, onUnsave, isSaved }: Props) {
  ...
  const handleCrearYGuardar = async () => {
    ...
    const { data: nueva } = await supabase
      ...
      .select('id, nombre')
      .single()
    if (nueva) { onSaved(nueva.id); onClose() }
    ...
    <div className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center">
      ...
      <div className="relative bg-white rounded-t-3xl sm:rounded-3xl shadow-xl w-full sm:max-w-sm mx-0 sm:mx-4 max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-gray-100">
          <h3 className="font-semibold text-gray-900">Guardar en carpeta</h3>
            ...
            <>
              ...
              >
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium text-gray-900">{isSaved ? 'Quitar de guardados' : 'Guardar'}</p>
                  <p className="text-xs text-gray-400">Sin clasificar</p>
                </div>
                {isSaved && <Check size={16} className="text-gray-900" />}
              </button>
              {carpetas.map(c => (
                <button key={c.id} onClick={() => handleSaveToBoard(c.id)} disabled={saving === c.id}
                  className="w-full flex items-center gap-3 px-3 py-3 rounded-2xl hover:bg-gray-50 transition-colors text-left">
                  <div className="w-10 h-10 bg-gray-100 rounded-xl flex items-center justify-center">
                    {saving === c.id
        ...
        <div className="px-3 pb-4 pt-2 border-t border-gray-100">
          {showInput ? (
            <div className="flex gap-2">
              <input type="text" placeholder="Nombre de la carpeta" value={nuevaNombre}
                onChange={e => setNuevaNombre(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleCrearYGuardar()}
                autoFocus className="flex-1 px-3 py-2.5 bg-gray-50 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-gray-200" />
              <button onClick={handleCrearYGuardar} disabled={creandoCarpeta || !nuevaNombre.trim()}
                className="px-4 py-2.5 bg-gray-900 text-white text-sm font-medium rounded-xl hover:bg-gray-800 disabled:opacity-50">
            ...
            <button onClick={() => setShowInput(true)}
              ...
              <p className="text-sm font-medium text-gray-700">Crear carpeta nueva</p>
            </button>
          )}
        </div>
      </div>

  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="max-w-sm w-full">
        <h1 className="text-2xl font-light text-center mb-1">Registro tatuador</h1>
        <p className="text-sm text-gray-400 text-center mb-8">Sube tu trabajo y consigue clientes</p>
        <form onSubmit={handleSubmit} className="space-y-4">
          <input type="text" placeholder="Nombre completo" value={form.nombre} onChange={e => setForm(f => ({ ...f, nombre: e.target.value }))} required
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="username" value={form.username} onChange={e => setForm(f => ({ ...f, username: e.target.value.toLowerCase().replace(/[^a-z0-9_-]/g, '') }))} required
          ...
          <button type="submit" disabled={loading} className="w-full py-3 bg-[#111] text-white rounded-xl text-sm font-medium hover:bg-black/80 disabled:opacity-50">
            {loading ? 'Creando...' : 'Crear cuenta de tatuador'}
          </button>
        </form>
      </div>
    </div>
